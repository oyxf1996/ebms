...
WebAboutContent=left(ClearHtml(rs("content")),80)
...
WebNewNewsList=WebNewNewsList&"<tr><td width='75%'>· <a href='"&rs_url&"' target='_blank' title='"&rs("title")&"'>"&left(rs("title"),23)&"</a></td>"
...
set rs=nothing
set rs=server.createobject("adodb.recordset")
sql="select top 10 [name],[folder],[id],[pid],[ppid] from [category] where pid="&ItemID&" and ClassType=2 order by [order] "
rs.open(sql),cn,1,1
if not rs.eof then
for i=1 to 10
 if i=2 then
...
Block03_LeftItem=Block03_LeftItem&"<div class='albumblock'><div class='inner'><a href='"&rs_url&"' target='_blank' title='"&rsp("title")&"'><img src='/images/up_images/"&rsp("image")&"' width='166' height='166'/><div class='albumtitle'>"&left(rsp("title"),14)&"</div></a></div></div>"
rsp.movenext
loop
end if 
rsp.close
